...
function Calendario({ usuario }) {
  ...
  const cargarCalendario = async () => {
    ...
    try {
      ...
      // Ensure the data is an array; backend might return an object with 'events' key
      ...
      days.push(
        <div key={d} className={`calendar-day ${isToday ? 'today' : ''}`}>
          ...
          <div className="event-indicators">
            {dayEvents.map((e, idx) => (
  <div key={idx} className={`event-dot ${e.esPropio ? 'mine' : 'others'} ${e.necesita_audiovisual ? 'av-req' : ''}`} />
            ))}
          </div>
        </div>
      );
