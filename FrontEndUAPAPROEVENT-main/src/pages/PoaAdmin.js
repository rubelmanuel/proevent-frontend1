...
export default function PoaAdmin({ usuario, searchTerm = "" }) {
  ...
  const handleCrearPoa = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/poa`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${usuario?.token || ""}`, "x-usuario-id": usuario?.id_usuario || "" },
        body: JSON.stringify({ fecha_inicio: fechaInicio, fecha_fin: fechaFin, monto_total: montoTotal })
      });
      if (res.ok) {
        setSuccessMsg("Presupuesto POA anual guardado con éxito.");
        setFechaInicio(""); setFechaFin(""); setMontoTotal("");
        cargarPoaData();
      } else {
        setErrorMsg("Error al crear POA");
      }
    } catch (e) {
  ...
  // UI messages
  {successMsg && <div className="alert-success" style={{ background: "#dcfce7", color: "#166534", padding: "10px", borderRadius: "6px", marginBottom: "10px" }}>{successMsg}</div>}
  {errorMsg && <div className="alert-error" style={{ background: "#fee2e2", color: "#991b1b", padding: "10px", borderRadius: "6px", marginBottom: "10px" }}>{errorMsg}</div>}
  return (
    <div className="admin-page-container fade-in">
        ...
        <div className="controls-header">
          <div className="title-section">
            ...
            <div>
              ...
              <p className="subtitle">Gestiona el presupuesto anual para los eventos de la universidad.</p>
            </div>
          </div>
          <div className="header-actions-group">
            <button 
              ...
              disabled={loading}
              title="Recargar datos del POA"
            >
              <FiRefreshCw className={loading ? "icon-spin" : "icon-spin-hover"} /> 
      ...
      <div className="dashboard-double-panel" style={{ marginTop: '20px' }}>
        <div className="saas-panel-card">
          <div className="panel-header">
            <h4>Aperturar Año Fiscal POA</h4>
          </div>
          <div className="panel-body">
            <form onSubmit={handleCrearPoa} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
              <div>
                <label style={{ display: "block", marginBottom: "5px", fontWeight: "600", fontSize: '13px' }}>Fecha de Inicio</label>
                <input type="date" value={fechaInicio} onChange={e => setFechaInicio(e.target.value)} required className="form-select" style={{ width: "100%", padding: "10px", borderRadius: "10px" }} />
              </div>
              <div>
                <label style={{ display: "block", marginBottom: "5px", fontWeight: "600", fontSize: '13px' }}>Fecha de Término</label>
                <input type="date" value={fechaFin} onChange={e => setFechaFin(e.target.value)} required className="form-select" style={{ width: "100%", padding: "10px", borderRadius: "10px" }} />
              </div>
              <div>
                <label style={{ display: "block", marginBottom: "5px", fontWeight: "600", fontSize: '13px' }}>Monto Aprobado (DOP)</label>
                <input type="number" step="0.01" value={montoTotal} onChange={e => setMontoTotal(e.target.value)} required placeholder="Ej. 1500000.00" className="form-select" style={{ width: "100%", padding: "10px", borderRadius: "10px" }} />
              </div>
              <button type="submit" className="btn-av-primary" disabled={loading}>Guardar Presupuesto POA</button>
            </form>
          </div>
        </div>
        ...
        <div className="saas-panel-card">
          <div className="panel-header">
            <h4>Resumen del POA</h4>
          </div>
          <div className="panel-body" style={{ display: 'flex', flexDirection: 'column', background: '#f8fafc', height: '100%' }}>
          {poaActual ? (
            <div style={{ textAlign: 'center' }}>
              <FiDollarSign size={40} color="#16a34a" />
              <h3 style={{ margin: "5px 0", color: "#334155" }}>Resumen del POA Activo</h3>
              <p style={{ color: "#64748b", fontSize: "12px" }}>Del {formatFecha(poaActual.fecha_inicio)} al {formatFecha(poaActual.fecha_fin)}</p>
      ...
      <div className="saas-panel-card" style={{ marginTop: '20px' }}>
        <div className="panel-header">
          <h4>Historial de Movimientos y Solicitudes del Poa</h4>
        </div>
            ...
            <tbody>
              {filteredMovimientos.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(mov => (
                <tr key={mov.id_movimiento}>
                  ...
                  <td><strong>#EVT-{mov.id_evento}</strong><br/><span style={{fontSize: "12px", color: "#64748b"}}>{mov.nombre_evento}</span></td>
                  <td>{mov.solicitante || "N/D"}</td>
                  <td>{Number(mov.monto_solicitado_original).toLocaleString("en-US", {minimumFractionDigits: 2})} {mov.moneda_original} <br/><span style={{fontSize: "10px", color: "#94a3b8"}}>Tasa: {mov.tasa_cambio}</span></td>
                  <td style={{ fontWeight: 'bold', color: '#ef4444' }}>- RD$ {Number(mov.monto_descontado_dop).toLocaleString("en-US", {minimumFractionDigits: 2})}</td>
                  <td>
                  <span style={{ 
                ...
                <td style={{ textAlign: 'center' }}>
                  <button className="reload-data-btn" onClick={() => openModalDetalles(mov)} style={{ width: 'auto', padding: '0 12px', gap: '6px', margin: '0 auto' }}>
                    <FiEye /> Ver
                  </button>
                </td>
                <td style={{ textAlign: 'center' }}>
